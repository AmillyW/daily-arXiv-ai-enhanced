  ...
  lines.forEach(line => {
    try {
      ...
      const formatText = (text) => {
        if (!text) return '';
        const mathBlocks = [];
        const save = (m) => { mathBlocks.push(m); return `\x00M${mathBlocks.length - 1}\x00`; };
        // 先把所有数学公式占位保护，避免换行替换破坏公式
        let t = text
          .replace(/\$\$[\s\S]*?\$\$/g, save)
          .replace(/\\\[[\s\S]*?\\\]/g, save)
          .replace(/\\\([\s\S]*?\\\)/g, save)
          .replace(/\$[^$\n]+?\$/g, save);
        // 换行符转 <br>（此时公式已被替换为占位符，不会被破坏）
        t = t.replace(/\\n/g, '<br>').replace(/\n/g, '<br>');
        // 还原数学公式
        t = t.replace(/\x00M(\d+)\x00/g, (_, i) => mathBlocks[parseInt(i)]);
        return t;
      };
      ...
      result[primaryCategory].push({
        ...
        date: date,
        id: paper.id,
        motivation: paper.AI && paper.AI.motivation ? formatText(paper.AI.motivation) : '',
        method: paper.AI && paper.AI.method ? formatText(paper.AI.method) : '',
        result: paper.AI && paper.AI.result ? formatText(paper.AI.result) : '',
        conclusion: paper.AI && paper.AI.conclusion ? formatText(paper.AI.conclusion) : '',
...
// 渲染页面中所有 LaTeX 公式（KaTeX auto-render）
function renderMath(element) {
  ...
  renderMathInElement(target, {
    ...
    // htmlAndMathml 模式输出 MathML annotation，复制时可还原源码
    output: 'htmlAndMathml',
    // 不因未知命令抛出错误，降级显示原始文本
    throwOnError: false,
    // 关闭严格模式，允许更宽松的公式语法
    strict: false,
    // 渲染前预处理：把双反斜杠还原为单反斜杠（JSON转义导致的问题）
    preProcess: (math) => math.replace(/\\\\/g, '\\')
  });
}

// 复制时将 KaTeX 渲染结果中的数学公式替换为 LaTeX 源码
document.addEventListener('copy', function(e) {
  ...
  katexEls.forEach(el => {
    ...
    if (annotation) {
      text = text.replace(el.textContent, '$' + annotation.textContent.trim() + '$');
    }
  });
