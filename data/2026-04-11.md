...
TL;DR: We introduce a Super-Grassmannian integral representation for calculating $n$-point functions in $\mathcal{N}=1$ SCFT$_3$, implementing conformal invariance, supersymmetry, and special superconformal invariance through delta function constraints. This leads to simple algebraic relations among component correlators, allowing the determination of any correlator from just one. We demonstrate the application of this formalism by constructing (A)dS$_4$ boundary correlators and calculating the (A)dS$_4$ Yang-Mills gluon four-point function from its gluino counterpart. The flat-space limit in super-space is also established, agreeing with existing results.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to develop a formalism that can effectively handle the complexities of $\mathcal{N}=1$ SCFT$_3$ while maintaining the necessary symmetries. This involves constructing an integral representation that can be used to calculate $n$-point functions, which are fundamental in understanding the dynamics of these theories. The goal is to provide a method that simplifies the computation of correlators and allows for the extraction of physical insights from the theory.

Method: We construct a Super-Grassmannian integral representation for $n$-point functions in $\mathcal{N}=1$ SCFT$_3$. Conformal invariance, supersymmetry, and special superconformal invariance are implemented through operator-valued delta function constraints. Simple algebraic relations among component correlators are obtained, allowing the determination of any component correlator from just one. The framework enables the construction of (A)dS$_4$ boundary correlators with contact diagrams from particle exchange contributions. The flat-space limit in super-space is also established.

Result: The result of this work is the development of a Super-Grassmannian integral representation that successfully implements conformal invariance, supersymmetry, and special superconformal invariance. This representation leads to simple algebraic relations among component correlators, enabling the determination of any correlator from just one. The framework is applied to construct (A)dS$_4$ boundary correlators with contact diagrams from particle exchange contributions, illustrated by the calculation of the (A)dS$_4$ Yang-Mills gluon four-point function from its gluino counterpart. The flat-space limit in super-space is also confirmed to agree with existing results.

Conclusion: The constructed Super-Grassmannian integral representation provides a powerful framework for calculating $n$-point functions in $\mathcal{N}=1$ SCFT$_3$. It allows for the manifest implementation of conformal invariance, supersymmetry, and special superconformal invariance through delta function constraints. The algebraic relations among component correlators enable the determination of any component correlator from just one, facilitating the construction of (A)dS$_4$ boundary correlators with contact diagrams from particle exchange contributions. This is demonstrated by the calculation of the (A)dS$_4$ Yang-Mills gluon four-point function from its gluino counterpart. Additionally, the flat-space limit in super-space is established, showing perfect agreement with existing results.
...
TL;DR: We develop a Super-Grassmannian formalism for $n-$point functions in $\mathcal{N}=2$ to $4$ SCFTs, incorporating super-conformal and $R-$symmetry constraints. We test this formalism in $\mathcal{N}=2$ and $\mathcal{N}=4$ AdS$_4$ super Yang-Mills theories, reproducing correlators and demonstrating its effectiveness, especially in the flat space limit where it matches with $\mathcal{N}=4$ SYM amplitudes.


<details>
  <summary>Details</summary>
Motivation: To provide a systematic approach for calculating correlation functions in supersymmetric conformal field theories (SCFTs) by leveraging the properties of super-conformal symmetry and $R-$symmetry.

Method: A Super-Grassmannian formalism is developed to describe $n-$point functions in $\mathcal{N}=2$ to $4$ SCFT$_3$. The constraints from super-conformal invariance and $R-$symmetry are incorporated through operator-valued delta functions. The formalism is tested in $\mathcal{N}=2$ and $\mathcal{N}=4$ AdS$_4$ super Yang-Mills theories, with specific constructions for the super-operator in both cases.

Result: The four-gluon correlator in $\mathcal{N}=2$ is reproduced using the four-point scalar correlator as input. For $\mathcal{N}=4$, two distinct constructions of the super-operator are presented: one including states up to spin two and another in a CPT self-conjugate manner, containing only operators with spin zero, spin half, and spin one. The latter construction aligns with flat space $\mathcal{N}=4$ SYM amplitudes, highlighting the framework's utility.

Conclusion: The constructed Super-Grassmannian framework successfully reproduces known correlators and demonstrates the non-triviality of the formalism, particularly in the flat space limit where it matches with $\mathcal{N}=4$ SYM amplitudes.
...
TL;DR: This paper extends the Sen action to include two metrics, enabling its application to generic manifolds. It addresses the challenges of Kaluza-Klein compactifications and shows that zero-modes are necessary for a consistent truncation. Despite the apparent increase in degrees of freedom, on-shell these do not lead to new physical states. Additionally, a deformation of the Sen action introduces an additional form-field that does not contribute new degrees of freedom on-shell but emerges naturally upon compactification.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the need to generalize the Sen action for self-dual fields to more complex geometries involving multiple metrics. This generalization is crucial for understanding the behavior of fields on generic manifolds and for exploring the implications of higher-dimensional theories through Kaluza-Klein compactifications.

Method: The method involves extending the Sen action to accommodate two metrics, analyzing the implications of Kaluza-Klein compactifications, and examining the consistency of truncations and the nature of degrees of freedom. The approach includes theoretical derivations and discussions on the mathematical structures involved.

Result: The results demonstrate that including zero-modes associated with each Kaluza-Klein tower in a consistent truncation prevents the naive doubling of massless degrees of freedom. Furthermore, a deformation of the Sen action introduces an additional form-field that does not add new on-shell degrees of freedom but arises naturally during the compactification process.

Conclusion: The paper explores the generalization of the Sen action to include two metrics, leading to a Kaluza-Klein compactification. It discusses the challenges posed by the existence of two Kaluza-Klein towers and shows that zero-modes must be included in a consistent truncation. Despite this seemingly doubling of massless degrees of freedom, on-shell these do not contribute new physical states. Additionally, it examines a deformation of the Sen action to incorporate an additional form-field, which does not introduce new degrees of freedom on-shell but emerges naturally upon compactification.
...
TL;DR: The paper extends Gaussian covariant hydrodynamics by incorporating torsion, leading to a fluctuating hydrodynamics with spin that is covariant under pseudo-gauge transformations and generally covariant with respect to foliations.


<details>
  <summary>Details</summary>
Motivation: The motivation for this work is to formulate a fluctuating hydrodynamics with spin that is covariant under pseudo-gauge transformations and generally covariant with respect to foliations. This addresses the need to clarify the role of the pseudo-gauge in hydrodynamics with spin by ensuring that angular momentum observables are covariantly dependent on the pseudo-gauge while the dynamics remains pseudo-gauge independent.

Method: The method involves extending the Gaussian covariant hydrodynamics approach using torsion as an auxiliary field. The second-order gravitational Ward identities are derived in the torsionful case to ensure covariance under pseudo-gauge transformations and general covariance with respect to foliations.

Result: The result is the formulation of a fluctuating hydrodynamics with spin that is covariant under pseudo-gauge transformations and generally covariant with respect to foliations. This is achieved through the use of torsion as an auxiliary field and the derivation of second-order gravitational Ward identities in the torsionful case.

Conclusion: The paper extends the Gaussian covariant hydrodynamics approach by incorporating torsion as an auxiliary field, resulting in a fluctuating hydrodynamics with spin that is covariant under pseudo-gauge transformations and generally covariant with respect to foliations. This is achieved through second-order gravitational Ward identities derived in the torsionful case, ensuring that angular momentum observables are covariantly dependent on the pseudo-gauge while the dynamics remains pseudo-gauge independent.
...
TL;DR: The paper analyzes the linearized deformations of thick-walled Q-balls, decomposing them into relativistic and Floquet modes. It offers closed-form solutions for most modes but not the bound mode at leading order.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand the dynamics of thick-walled Q-balls by analyzing their linearized deformations. This includes identifying and describing the different types of modes that arise from these deformations, such as relativistic modes and Floquet modes, and providing a mathematical framework to describe them.

Method: The method involves analyzing the linearized deformations of Q-balls, decomposing them into relativistic modes and Floquet modes. The paper uses perturbation theory to expand the modes to leading order in the Q-ball amplitude and presents all modes in closed form except for the bound mode which does not exist at leading order.

Result: The results include the decomposition of linearized deformations of thick-walled Q-balls into relativistic modes and Floquet modes. The paper provides closed-form expressions for corotating and counterrotating modes, with the latter described by an irrational-level Pöschl-Teller potential. The bound mode is not present at leading order and thus not expressed in closed form.

Conclusion: The paper presents a detailed analysis of the linearized deformations of thick-walled Q-balls in (1+1)-dimensional space, decomposing them into relativistic modes and Floquet modes. It provides closed-form expressions for most modes except the bound mode at leading order.
...
TL;DR: We investigate the ODE/IM correspondence for $E_6^{(1)}$ and find that the eigenvalues of the integrals of motion match the period integrals up to the sixth order.


<details>
  <summary>Details</summary>
Motivation: To explore the connection between the ordinary differential equation (ODE) and the integrable model (IM) for the affine Lie algebra $E_6^{(1)}$, and to verify the agreement between the eigenvalues of the integrals of motion and the period integrals up to the sixth order.

Method: We study the ODE/IM correspondence for the ordinary differential equation associated with the affine Lie algebra $E_6^{(1)}$. The WKB expansion of the solution of the ODE is performed by the diagonalization method, and the period integrals of the WKB coefficients along the Pochhammer contour are calculated. We also compute the integrals of motion on a cylinder in two-dimensional conformal field theory with W-symmetry associated with $E_6^{(1)}$. Their eigenvalues on the highest-weight state are shown to agree with the period integrals up to the sixth order.

Result: The eigenvalues of the integrals of motion on a cylinder in two-dimensional conformal field theory with W-symmetry associated with $E_6^{(1)}$ are computed and found to agree with the period integrals up to the sixth order.

Conclusion: The eigenvalues of the integrals of motion on a cylinder in two-dimensional conformal field theory with W-symmetry associated with $E_6^{(1)}$ agree with the period integrals up to the sixth order.
...
### [19] [Leading low-temperature correction to the Heisenberg-Euler Lagrangian](https://arxiv.org/abs/2604.07996)
*Felix Karbstein*
...
TL;DR: This paper introduces a BF formulation for two-dimensional dilaton gravity that enables the study of higher-spin algebras, revealing an infinite set of scalar degrees of freedom with increasing masses. It also proposes a way to incorporate scalar field backreaction into the gravity sector, offering a new example of a fully interacting higher-spin gravity theory in two dimensions without a cosmological constant.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop a BF formulation that can accommodate higher-spin symmetries in two-dimensional dilaton gravity, leading to the identification of new degrees of freedom and the possibility of constructing a fully interacting higher-spin gravity theory. This work aims to provide a theoretical framework for understanding the dynamics of higher-spin systems in low-dimensional spacetimes.

Method: The method involves using a BF formulation of two-dimensional dilaton gravity to define gauge theories of higher-spin algebras. The authors analyze the spectrum of two-dimensional higher-spin gravity with a vanishing cosmological constant based on an extended, infinite-dimensional higher-spin algebra. They identify an infinite collection of scalar degrees of freedom with increasing masses and discuss the inclusion of backreaction effects.

Result: The results include the demonstration of an infinite collection of scalar degrees of freedom with a continuum of ever-increasing mass in the spectrum of two-dimensional higher-spin gravity with a vanishing cosmological constant. These degrees of freedom correspond to the twisted-(co)adjoint representation of the higher-spin algebra. Additionally, the authors propose a method to include backreaction effects of scalar fields on the gravity sector, which is a novel aspect in the context of higher-spin gravity theories.

Conclusion: The paper presents a BF formulation of two-dimensional dilaton gravity that allows for the definition of gauge theories of higher-spin algebras, including their finite and infinite-dimensional generalizations. It demonstrates the existence of an infinite collection of scalar degrees of freedom with increasing masses in the spectrum of two-dimensional higher-spin gravity with a vanishing cosmological constant, corresponding to the twisted-(co)adjoint representation. The authors also discuss an approach to incorporate backreaction of scalar fields on the gravity sector at the formal level of equations of motion, providing a first example of a fully interacting higher-spin gravity theory in two dimensions with no cosmological constant.
...
TL;DR: In this paper, we extend the Grassmannian construction to incorporate a kinematic prefactor under $\mathcal{N}=2$ supersymmetry, enabling a complete description of wave function coefficients for conserved currents. This results in a unified formula that distinguishes between spinning and non-spinning cases and yields distinct helicity amplitudes in the flat-space limit.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the need to provide a comprehensive description of wave function coefficients (WFCs) for conserved currents, which are subject to inhomogeneous Ward identities. The existing Grassmannian construction only reproduces the discontinuities of these WFCs, leaving out the continuous parts. By incorporating $\mathcal{N}=2$ supersymmetry, we aim to resolve this issue and obtain a complete WFC description.

Method: By leveraging $\mathcal{N}=2$ supersymmetry, we related spinning and non-spinning WFCs, resulting in a Grassmannian formula with an additional kinematic prefactor. This prefactor is essential for capturing the full WFCs, including those for conserved currents which satisfy inhomogeneous Ward identities.

Result: The result is a Grassmannian formula for WFCs that includes a kinematic prefactor, which together capture the full WFCs for both spinning and non-spinning cases. This formula is derived under the framework of $\mathcal{N}=2$ supersymmetry and provides a unified approach to dealing with inhomogeneous Ward identities. Additionally, the positive and negative branches of this formula yield distinct helicity amplitudes in the flat-space limit.

Conclusion: We have extended the Grassmannian construction to include a kinematic prefactor that captures the full wave function coefficients (WFCs) for conserved currents, thereby resolving the challenge posed by inhomogeneous Ward identities. This leads to a unified formula that encompasses both spinning and non-spinning WFCs under $\mathcal{N}=2$ supersymmetry. The positive and negative branches of this augmented Grassmannian formula provide distinct helicity amplitudes in the flat-space limit.
...
### [28] [LSST Strong Lensing Systems Dark Matter Sensitivity Analysis with Neural Ratio Estimators](https://arxiv.org/abs/2604.07438)
...
### [29] [A Natural $\gtrsim 100\times$ Telescope: Discovery of the Strongly Lensed Type II SN 2025mkn at $z=1.37$](https://arxiv.org/abs/2604.07983)
...
### [30] [The peculiar velocity correlation function of the Cosmicflows-4 catalog](https://arxiv.org/abs/2604.08314)
...
### [31] [A Dynamical Equilibrium Linking Nanohertz Stochastic Gravitational Wave Background to Cosmic Structure Formation](https://arxiv.org/abs/2604.08317)
...
### [32] [Ray-traced weak lensing convergence in screened modified gravity theories](https://arxiv.org/abs/2604.08393)
...
### [33] [Coupled Dark Energy and Dark Matter for DESI: An Effective Guide to the Phantom Divide](https://arxiv.org/abs/2604.08449)
...
### [34] [A GPU-Accelerated JAX Framework for Robust Parametric Component Separation and Clustering Optimization for CMB Polarization Satellites](https://arxiv.org/abs/2604.08463)
...
### [35] [Probing non-Gaussianity during reheating with SIGW in the LISA band](https://arxiv.org/abs/2604.08493)
*Gabriele Perna,Guillem Domènech*

Main category: astro-ph.CO

TL;DR: We investigate how non-standard reheating affects the SIGW spectrum, finding characteristic features detectable by LISA. The reheating dynamics can either diminish or amplify the signal, impacting its detectability. A Fisher forecast is used to assess detectability for different parameters.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand how the reheating epoch, characterized by non-standard evolution, impacts the spectrum of SIGWs. This is crucial for interpreting potential detections by future interferometers like LISA, which aim to probe the early universe's gravitational wave background.

Method: We analyze the effects of a non-standard evolution of the Universe during the reheating epoch on the spectrum of scalar-induced gravitational waves (SIGWs), accounting for the presence of primordial non-Gaussianity. We perform a Fisher forecast for different values of $w$ and different scans to assess the detectability of the signal when considering various amplitudes and central frequencies.

Result: We find that given values of $w$ and $c_s^2$ leave distinctive signatures in the SIGW spectrum, which are detectable by LISA. The specific reheating dynamics can either suppress or enhance this spectrum, affecting its detectability. A Fisher forecast is performed for various $w$ values and scans to evaluate the signal's detectability under different amplitude and central frequency scenarios.

Conclusion: We conclude that the specific reheating dynamics can significantly affect the spectrum of scalar-induced gravitational waves, leading to characteristic features that can be detected by third-generation interferometers such as LISA. The detectability of these signals depends on the values of $w$ and $c_s^2$, with some configurations potentially enhancing or suppressing the signal strength.
...
### [36] [The Heavy Tailed Non-Gaussianity of the Supermassive Black Hole Gravitational Wave Background](https://arxiv.org/abs/2604.08506)
...
### [37] [Disentangling cosmic distance tensions with early and late dark energy](https://arxiv.org/abs/2604.08530)
...
### [38] [Phenomenology of an Open Effective Field Theory of Dark Energy](https://arxiv.org/abs/2603.12321)
...
### [39] [Finite Volume Effects on Transverse Momentum Spectra at LHC and RHIC Using a Blast-Wave Model with Planck Transformed Temperatures](https://arxiv.org/abs/2604.07410)
...
### [40] [Scalars at the Cosmological Collider: Full Shapes of Tree Diagrams and Bispectrum Searches using Planck Data](https://arxiv.org/abs/2604.07434)
...
### [41] [Gauged Q-balls in flat potentials](https://arxiv.org/abs/2604.07454)
*Julian Heeck,Yu Zhi*
...
### [42] [GOOFy fermions](https://arxiv.org/abs/2604.07458)
*P. M. Ferreira*

Main category: hep-ph

TL;DR: A new class of symmetries in two Higgs doublet models was discovered through an unorthodox transformation. The paper derives fermion field transformations respecting these symmetries and finds new parameter space regions invariant under renormalization.


<details>
  <summary>Details</summary>
Motivation: The motivation is to explore a new class of symmetries in two Higgs doublet models, which were recently discovered through an unorthodox transformation on scalar and gauge fields as well as spacetime coordinates. This work aims to extend the understanding by establishing the corresponding fermion field transformations and identifying new regions of parameter space that remain invariant under renormalization to all orders of perturbation theory.

Method: The method involves analyzing scalar and gauge fields along with spacetime coordinates through an unorthodox transformation. The Yukawa matrix textures are shown to respect these symmetries up to two-loops. The fermion field transformations are derived within this context.

Result: The result includes the derivation of fermion field transformations for the two Higgs doublet models under the new symmetries. It also identifies new regions of parameter space that are invariant under renormalization to all orders of perturbation theory, including scalar-fermion interactions.

Conclusion: The paper establishes the fermion field transformations for two Higgs doublet models under newly discovered symmetries, leading to the discovery of new regions of parameter space invariant under renormalization to all orders of perturbation theory.
...
### [43] [Axion-like Particles and Lepton Flavor Violation in Muonic Atoms](https://arxiv.org/abs/2604.07481)
...
### [44] [Excitation function for global Λpolarization in relativistic heavy ion collisions with the Core Corona model](https://arxiv.org/abs/2604.07501)
...
### [45] [Forward trijet production in proton-nucleus collisions: gluon initiated channel](https://arxiv.org/abs/2604.07509)
...
### [46] [Lecture notes on Machine Learning applications for global fits](https://arxiv.org/abs/2604.07520)
*Jorge Alda*
...
### [47] [Transverse energy-momentum tensor distributions in polarized nucleons](https://arxiv.org/abs/2604.07616)
*Ho-Yeon Won,Cédric Lorcé*

Main category: hep-ph

TL;DR: We use the quantum phase-space formalism to study the relativistic energy-momentum tensor in polarized nucleons, focusing on transverse-index components and their multipole structure. The formalism reproduces standard light-front distributions, including 'bad' components, and explains their structure in the infinite-momentum frame.


<details>
  <summary>Details</summary>
Motivation: To investigate the relativistic spatial distributions of the energy-momentum tensor inside polarized nucleons using the quantum phase-space formalism. The motivation is to understand the components involving at least one transverse index and their multipole structure, as well as to confirm the formalism's ability to reproduce standard light-front distributions, including those with a 'bad' component.

Method: The study employs the quantum phase-space formalism to analyze the relativistic spatial distributions of the energy-momentum tensor within polarized nucleons. The focus is on the components of the energy-momentum tensor with at least one transverse index, and the multipole structure of these distributions in a moving nucleon is explored. The infinite-momentum frame is utilized to compare the results with standard light-front distributions.

Result: The quantum phase-space formalism successfully reproduces the standard light-front distributions for the energy-momentum tensor components with at least one transverse index, including those with a 'bad' component. This includes the origin of their structure, which is explained through the formalism's application in the infinite-momentum frame.

Conclusion: The quantum phase-space formalism is shown to be capable of reproducing the standard light-front distributions for the energy-momentum tensor components involving at least one transverse index, including those with a 'bad' component. This confirms the formalism's validity and provides insights into the structure of these distributions.
...
### [48] [Multi Component Dark Matter in a Minimal Model](https://arxiv.org/abs/2604.07618)
*Karim Ghorbani*
...
### [49] [Memory effect on the heavy quark dynamics in hot QCD matter](https://arxiv.org/abs/2604.07982)
*Jai Prakash,Ling Hai Li,Ying Shan Zhao,Yifeng Sun*

Main category: hep-ph

TL;DR: This paper examines the impact of memory on heavy quark dynamics in the quark gluon plasma using a generalized Langevin equation with time correlated thermal noise modeled by a fractional differential equation. The findings highlight the substantial role of memory in modifying various physical observables related to heavy quark behavior.


<details>
  <summary>Details</summary>
Motivation: To investigate how memory effects, characterized by time correlated thermal noise, modify heavy quark dynamics within the quark gluon plasma. This is crucial for understanding the behavior of heavy quarks in high-energy collisions, which can provide insights into the properties of the early universe.

Method: The research employs a generalized Langevin equation to model heavy quark dynamics under the influence of memory. Time correlated thermal noise is modeled using a fractional differential equation with the Caputo fractional derivative of order parameter $ν$. The memory effect is analyzed through the aforementioned physical observables.

Result: The results show that the memory effect, represented by the time correlated thermal noise, has a significant impact on the heavy quark dynamics. This includes changes in the momentum correlation, average squared momentum, average squared displacement, and average kinetic energy. Furthermore, the higher normalised central moments of the transverse-momentum distribution are also influenced by the memory effect.

Conclusion: The study reveals that time correlated thermal noise, generated by a fractional differential equation with Caputo fractional derivative, significantly impacts heavy quark dynamics in the quark gluon plasma. The memory effect is quantified through various physical observables such as momentum correlation, average squared momentum, displacement, and kinetic energy. Additionally, the higher normalised central moments of the transverse-momentum distribution are also affected by the memory.
...
### [50] [Constraining Ultralight Scalar Dark Matter in the Galactic Center with the S2 Orbit](https://arxiv.org/abs/2604.08053)
...
### [51] [Probing Majoron Dark Matter with Gravitational Wave Detectors](https://arxiv.org/abs/2604.08193)
...
### [52] [Fermion Multiplicities at the GUT Scale: A Statistical Study of Unification and Proton Decay](https://arxiv.org/abs/2604.08237)
...
### [53] [$\bar B\to D^{(*)}\ell\bar ν$ Branching Ratios and Evidence for Isospin Breaking in $Υ(4S)$ Decays](https://arxiv.org/abs/2604.08391)
...
### [54] [LFV decays in a 3-4-1 model with minimal inverse seesaw neutrinos](https://arxiv.org/abs/2604.08444)
*N. H. T. Nha,L. T. Hue,L. T. T. Phuong,T. T. Hong*

Main category: hep-ph

TL;DR: An extended 3-4-1 model with a new singly charged Higgs boson and the minimal inverse seesaw mechanism is proposed to explain $(g-2)_{e,μ}$ anomalies and lepton-flavor-violating decay rates, predicting strong observable correlations testable in future experiments.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the $(g-2)_{e,μ}$ anomalies and lepton-flavor-violating decay rates observed in the Standard Model, which are not well explained by the current theory. By extending the 3-4-1 model and introducing a new singly charged Higgs boson along with the minimal inverse seesaw mechanism, the paper aims to provide a framework that can better fit these experimental observations.

Method: The method involves constructing an extended 3-4-1 model incorporating a new singly charged Higgs boson and applying the minimal inverse seesaw mechanism. This approach allows for the accommodation of large $(g-2)_{e,μ}$ anomalies and lepton-flavor-violating decay rates, all within the bounds of current experimental data.

Result: The result shows that the extended 3-4-1 model with the new singly charged Higgs boson and the minimal inverse seesaw mechanism can explain the $(g-2)_{e,μ}$ anomalies and lepton-flavor-violating decay rates. Additionally, it predicts strong correlations among these observables, which can be tested in future experiments. Specifically, the current upper bound on Br$(τ\to μγ)$ imposes a stringent constraint compatible with the $1σ$ experimental range of $(g-2)_μ$, indicating a maximal deviation of $10^{-9}$ from the Standard Model prediction. Future experimental sensitivities will reduce this deviation to $5\times 10^{-10}$. 

Conclusion: The extended 3-4-1 model with a new singly charged Higgs boson and the minimal inverse seesaw mechanism successfully accommodates the $(g-2)_{e,μ}$ anomalies and lepton-flavor-violating decay rates, while maintaining consistency with current experimental data. The model predicts strong correlations among these observables that can be tested in future experiments, particularly through improvements in the sensitivity to Br$(τ\to μγ)$.
...
### [55] [Kinetic and canonical momentum broadening in the Glasma](https://arxiv.org/abs/2604.08520)
*Dana Avramescu,Carlos Lamas,Tuomas Lappi,Meijian Li,Carlos A. Salgado*

Main category: hep-ph

TL;DR: This paper develops a quantum formalism for describing the real-time evolution of particles in the Glasma phase of heavy-ion collisions, emphasizing gauge invariance and momentum definitions. It derives equations of motion for kinetic and canonical momenta, demonstrating the impact of transverse field components and the benefits of using a transverse Coulomb gauge condition to minimize numerical errors.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the need to develop a quantum formalism that accurately describes the real-time evolution of particles in the Glasma phase of heavy-ion collisions. This requires addressing the implications of gauge invariance in defining particle momentum within a classical background field. The work aims to provide a robust theoretical foundation for future quantum calculations in this context.

Method: The method involves establishing the correspondence between classical Wong's equations and Heisenberg equations for particles in a classical non-Abelian background field. This leads to the derivation of equations of motion for both kinetic and canonical momenta. The analysis includes the impact of transverse field components on kinetic momentum broadening and the optimization of numerical accuracy through the use of a transverse Coulomb gauge condition.

Result: The results include the derivation of equations of motion for both kinetic and canonical momenta, highlighting the role of transverse field components in kinetic momentum broadening. The imposition of a transverse Coulomb gauge condition at initial time is shown to reduce numerical errors, thereby improving the overall accuracy of the quantum implementation.

Conclusion: The paper establishes a quantum formalism for the real-time evolution of particles in the Glasma phase, focusing on gauge invariance and momentum definitions. It demonstrates the correspondence between classical Wong's equations and Heisenberg equations, leading to equations of motion for both kinetic and canonical momenta. The kinetic momentum broadening is shown to receive contributions from transverse field components even in the eikonal limit. Additionally, imposing a transverse Coulomb gauge condition at initial time significantly reduces numerical errors, optimizing the framework for quantum implementation.
...
### [56] [Electromagnetic wave propagation in static black hole spacetimes: an effective refractive index description in Schwarzschild geometry](https://arxiv.org/abs/2604.07371)
...
### [57] [Exact quasinormal residues and double poles from hypergeometric connection formulas](https://arxiv.org/abs/2604.07400)
*Ye Zhou*

Main category: gr-qc

TL;DR: We present a unified mathematical method for analyzing the pole structure of frequency-domain Green's functions and quasinormal modes in radial boundary value problems, using connection formulas for Kummer solutions to construct an explicit quantization function. This method avoids integral evaluation and provides an algebraic criterion for double-pole quasinormal modes.


<details>
  <summary>Details</summary>
Motivation: To develop a unified mathematical method for analyzing the pole structure of frequency-domain Green's functions and associated quasinormal spectra in radial boundary value problems, which are reducible to the Gauss hypergeometric equation.

Method: A systematic application of connection formulas for Kummer solutions to construct an explicit quantization function that encodes arbitrary linear asymptotic boundary conditions. The frequency-dependent spectral factor is controlled by the closed-form Digamma derivative of this quantization function, avoiding integral evaluation.

Result: The method successfully demonstrates the vanishing of the quantization function and its first derivative as a direct algebraic criterion for double-pole QNMs. It is benchmarked against the exact BTZ black hole spectrum and provides an analytic diagnostic for exceptional lines and nearly double-pole excitations in the Nariai/Pöschl-Teller limit.

Conclusion: The developed formalism provides a unified approach to the pole structure of frequency-domain Green's functions and quasinormal modes (QNMs) in radial boundary value problems, offering an algebraic criterion for double-pole QNMs.
...
### [58] [Consistency of the LQG quantization of black holes coupled with scalar matter and a clock](https://arxiv.org/abs/2604.07499)
*Rodrigo Eyheralde,Rodolfo Gambini*

Main category: gr-qc

TL;DR: The paper explores the gauge-fixed quantization of spherically symmetric gravity coupled to a scalar field in Loop Quantum Gravity, showing that it aligns with the Dirac method for black hole quantization, thereby resolving issues related to constraint algebra consistency.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the unresolved challenge in Dirac quantizing spherically symmetric gravity coupled to a scalar field within Loop Quantum Gravity due to difficulties in maintaining a consistent constraint algebra at the quantum level. The authors aim to overcome this by employing a gauge-fixing approach and ensuring the consistency of the resulting theory.

Method: The authors analyze the gauge-fixed quantization by comparing it to the well-known results for the quantization of a black hole in vacuum using the Dirac method. This involves a detailed treatment valid throughout the outer region of the black hole, where previous asymptotic approximations are not applicable.

Result: The gauge-fixed quantization reproduces the expected results for the quantization of a black hole in vacuum, demonstrating that the approach is valid and consistent with the Dirac method. This is achieved through a rigorous analysis that extends beyond the regions where previous studies relied on asymptotic approximations.

Conclusion: The gauge-fixed quantization approach is shown to be consistent with the Dirac method for the quantization of a black hole in vacuum, addressing the issue of maintaining a consistent constraint algebra at the quantum level.
...
### [59] [The fall and the rise of Weyl gauge theory](https://arxiv.org/abs/2604.07508)
*D. M. Ghilencea*
...
### [60] [Accretion Disks in Schwarzschild-MOG and Kerr-MOG Backgrounds: MOG Parameter in terms of Observational Quantities](https://arxiv.org/abs/2604.07597)
...
### [61] [Coalescing Compact Binary Parameter Estimation with Gravitational Waves in the Presence of non-Gaussian Transient Noise](https://arxiv.org/abs/2604.07668)
...
### [62] [Unveiling Inner Shadows and Polarization Signatures of Rotating Einstein-Gauss-Bonnet Black Holes](https://arxiv.org/abs/2604.07726)
...
### [63] [Quasinormal modes of the thick braneworld in $f(T)$ gravity](https://arxiv.org/abs/2604.07858)
...
### [64] [Observational Quantities in Quasi-Newtonian Descriptions of Cosmological Space-Times](https://arxiv.org/abs/2604.08119)
...
### [65] [Detecting Chiral Gravitational Wave Background with a Dipole Pulsar Timing Array](https://arxiv.org/abs/2604.08141)
*Baoyu Xu,Hanyu Jiang,Rong-Gen Cai,Misao Sasaki,Yun-Long Zhang*

Main category: gr-qc

TL;DR: This paper introduces a dipole pulsar timing array (dPTA) system to improve the detection of chiral gravitational wave backgrounds (GWBs) in the nanohertz regime, extending the detectable frequency range from nanohertz to microhertz.


<details>
  <summary>Details</summary>
Motivation: Conventional PTAs are limited in their ability to detect parity violation in GWBs due to their lack of sensitivity to such effects. The motivation behind this work is to develop a new system, the dPTA, which can overcome this limitation by enhancing the detection capabilities for chiral GWBs in the nanohertz regime.

Method: The method involves deriving overlap reduction functions (ORFs) from the cross-correlation of timing signals and analyzing the sensitivity curves of the dPTA system. This approach allows the detection of chiral GWBs in the nanohertz regime and demonstrates an extended detectable frequency range.

Result: The results show that the dPTA exhibits sensitivity to chiral GWBs in the nanohertz regime through the analysis of ORFs derived from cross-correlation. Numerical calculations reveal that the dPTA extends the detectable frequency range of PTAs for GWBs from the nanohertz to the microhertz regime, significantly improving the detection capabilities of PTAs.

Conclusion: The dipole pulsar timing array (dPTA) system is proposed to enhance the sensitivity of pulsar timing arrays (PTAs) for detecting chiral gravitational wave backgrounds (GWBs) in the nanohertz regime. Numerical calculations show that the dPTA extends the detectable frequency range from the nanohertz to the microhertz regime, thereby improving the capability of PTAs to probe parity violation in GWBs.
...
### [66] [Thermodynamics and orbital structure of anti-de Sitter black holes in Palatini-inspired nonlinear electrodynamics](https://arxiv.org/abs/2604.08170)
...
### [67] [GW231123: False Massive Graviton Signatures from Unmodeled Point-Mass Lensing](https://arxiv.org/abs/2604.08179)
...
### [68] [Positivity of holographic energy](https://arxiv.org/abs/2604.08183)
*Piotr T. Chruściel,Raphaela Wutte*

Main category: gr-qc

TL;DR: This paper proves the positivity of a weighted holographic energy for specific types of four-dimensional spacetimes with a negative cosmological constant and a conformally static boundary. The proof uses advanced mathematical methods from conformal field theory and renormalization group analysis.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this work is to understand the properties of spacetimes with specific geometric and topological features, particularly in the context of holography and the AdS/CFT correspondence.

Method: The proof involves advanced techniques from mathematical physics, including the use of conformal field theory and the analysis of renormalization group flows.

Result: The result is the demonstration of the positivity of a weighted holographic energy, which is a key quantity in the study of holographic principles and the dynamics of spacetime near the conformal boundary.

Conclusion: We prove positivity of a weighted holographic energy for four-dimensional spacetimes with negative cosmological constant whose conformal boundary at infinity is conformally static and admits either spherical sections, or toroidal sections with compatible spin structure.
...
### [69] [Dynamics for Spin-$1/2$ Particles in Einstein-Gauss-Bonnet Gravity](https://arxiv.org/abs/2604.08202)
*E. Maciel*
...
### [70] [Unifying topological, geometric, and complex classifications of black hole thermodynamics](https://arxiv.org/abs/2604.08315)
*Shi-Hao Zhang,Shao-Wen Wei,Jing-Fei Zhang,Xin Zhang*

Main category: gr-qc

TL;DR: Three classification schemes of black hole thermodynamics are unified through two dictionaries, simplifying analysis and providing a foundation for complex black holes.


<details>
  <summary>Details</summary>
Motivation: Black hole thermodynamics has recently witnessed three distinct classification schemes: based on local geometric properties of the temperature function, global topological invariants, and Riemann surface foliations in the complex plane. These schemes are shown to be equivalent in the real domain via two dictionaries.

Method: We show that the three distinct classification schemes of black hole thermodynamics are equivalent in the real domain via two dictionaries: one linking thermal stability to the monotonicity of the temperature curve, and the other connecting the number of black hole states to the foliation number of a Riemann surface. The number of extremal points of the temperature curve determines the classification in all three frameworks, tracing this unification to the critical point structure of the black hole solution space.

Result: Several black holes demonstrate how counting extrema yields topological invariants and phase transition information.

Conclusion: This unified framework simplifies black hole thermodynamic analysis and provides a foundation for exploring more complex black holes.
...
### [71] [Pyramid Interferometers: Direct Access to Cosmological Gravitational Wave Chirality](https://arxiv.org/abs/2604.08471)
...
### [72] [Metric affine gravity with dynamical chronology protection](https://arxiv.org/abs/2604.08511)
